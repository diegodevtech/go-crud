...
The Go-Crud application offers REST endpoints for creating, listing, updating, and deleting users. You can use tools like [curl](https://curl.se/) or [Postman](https://www.postman.com/) to test the endpoints. Here are some `curl` command examples for testing the endpoints:
...
- `password` (string, required): The user's password (must be at least 8 characters and contain at least one of the characters: !@#$%&*()_-=+).

### request.UserRequest
Structure containing the required fields for creating a new user.

- `age` (integer, required): The user's age (must be between 18 and 100).
- `email` (string, required): The user's email (must be a valid email address).
...
If you wish to contribute to the Go-Crud project, feel free to submit pull requests or report issues on the [official repository](https://github.com/diegodevtech/go-crud.git).
